...
async function setGroupMap() {
    for (const [examId, examInfo] of NCKURegisterInfo.exam) {    
        //1700816055每學期不同須更新
        const url = 'https://nbk.acad.ncku.edu.tw/netcheckin/views/js/code1.js?1763491827'; 
        ...
        for (let i = 2; i < resultArr.length; i++) {
            if (resultArr[i].includes("new Array();")) {
                continue;
            } else if (examId=='1' && resultArr[i].includes("A1['1']")) {
                    examNo = '1';
            } else if (examId=='2' && resultArr[i].includes("A1['2']")) {
                    examNo = '2';
...
export async function updateGroupsInfo() {
    for (const [groupId, groupInfo] of NCKURegisterInfo.group) {
        ...
        let want = 0;
        let currentReserve = "";
        const header0 = $("table thead tr th:eq(0)").text().trim();
        const header1 = $("table thead tr th:eq(1)").text().trim();
